...
| Variable | Required | Description | Example |
|----------|----------|-------------|---------|
| `PANGOLIN_EMAIL_SMTP_PASS` | ❌ | SMTP password (alternative to config.yml) | `your-smtp-password` |

:::note
Most SMTP settings are configured in `config/config.yml` rather than environment variables. Use `PANGOLIN_EMAIL_SMTP_PASS` for enhanced security to avoid storing passwords in config files.
:::
