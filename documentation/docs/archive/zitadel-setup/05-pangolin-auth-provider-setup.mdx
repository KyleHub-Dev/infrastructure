...
- ✅ Use **external HTTPS URL** (through Cloudflare)
...
- ✅ Use **internal HTTP URL with port 8080**
...
**Incorrect:** `urn:zitadel:iam:org:project:roles` → Parser error
**Correct:** `"urn:zitadel:iam:org:project:roles"` → Works correctly
...
1. Navigate to **Server Admin → Identity Providers**
...
- Zitadel organization: `CustomerA` → Pangolin organization: `CustomerA`
- Zitadel organization: `CustomerB` → Pangolin organization: `CustomerB`
...
Without a Project Grant, users in `CustomerA` organization cannot access the application you defined in your `KHOps` organization- even if they have valid Zitadel credentials.
...
**Result**: One application, many tenants, automatic routing- true multi-tenancy.
...
5. Navigate to **Projects → KyleHub** (in your management org)
...
1. Navigate to **Server Admin → Organizations**
...
Access control is always role-based- Pangolin checks if the authenticated user has the required role in their Zitadel token, regardless of which organization they belong to.
...
1. Go to **Server Admin → Roles** (or within your organization's settings)
2. Click **Create Role**
...
> 💡 **Category + service pattern**: By including both `access_adguard` and `admin`, you allow access via either a service-specific role or the broader admin role.
...
1. Navigate to **Server Admin → Roles** (or your organization's role management)
...
**Solutions**:
1. In **Server Admin → Users**, find the user and manually promote them to **Administrator** (see Section 8.4).
...
1. Verify the Project Grant exists:
   - In Zitadel, navigate to **Projects → KyleHub → Grants**
...
3. Verify the Organization Mapping Path in Pangolin:
   - **Server Admin → Identity Providers → Zitadel → Organization Policies**
...
1. ✅ Zitadel: Create org → Grant project access
2. ✅ Pangolin: Create matching org
3. ✅ Zitadel: Add users → Assign roles
4. ✅ Test: User logs in → Auto-routed to their org
