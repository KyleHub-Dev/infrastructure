...
2. Click on the resource to edit (e.g., your Documentation site)
3. Navigate to the **Authentifizierung** (Authentication) tab.

### 2. Configure Access Control

Configure the following settings to enforce Zitadel authentication:

| Setting | Action |
|---------|-------------|
| **Plattform SSO verwenden** | Toggle **ON** |
| **Standard Identitätsanbieter** | Select **Zitadel** |

### 3. Restricting Access by Role

To ensure only specific users (like Administrators) can access the resource:

1. In the **Rollen (Roles)** dropdown, select the roles that should have access (e.g., `ADMIN`).
2. If you want to allow specific individuals regardless of their role, you can add them in the **Benutzer (Users)** dropdown.
3. Click **Zugriffskontrollen speichern** (Save Access Controls).

:::tip Validation
Pangolin checks for the role key (e.g., `ADMIN`) in the token provided by Zitadel. Ensure this role is assigned to your user in the Zitadel project authorizations.
:::

---

## The "Regeln" (Rules) Tab

For standard role-based access control, you **can leave the Regeln tab empty**. This section is intended for advanced scenarios, such as:
- Mapping internal organization IDs.
- Complex JMESPath extractions for multi-tenant environments.
