...
> 📌 **Key mental model:** Zitadel asserts *who a user is* and *which roles they own*. Each application (Pangolin, Portainer, Grafana, …) decides what those roles allow.
...
- Category roles grant broad access (e.g., `homelab` → all homelab services)
- Service roles grant specific access (e.g., `access_grafana` → Grafana only)
...
- Simplifying grants (one role → many services)
...
| **Media Family Member** | `media` | Jellyfin, Sonarr, Radarr, Plex- nothing else |
...
> 🔁 **Renaming roles:** Keys are immutable once assigned. To rename, create the new role, update all user grants, then delete the old role.

> 💡 **Start small:** Begin with `admin`, `homelab`, and 3-5 service roles. Expand as you add services.
...
1. In Zitadel Console, open **Projects → KyleHub → Settings**.
...
1. Stay inside your project and navigate to **Applications → KyleHub-Auth**.
...
Initiate a normal Pangolin login (or the console’s built-in test login, if enabled). Copy the ID token and decode it with a tool such as [jwt.io](https://jwt.io/). You should now see a claim similar to:
...
> 💡 **Multiple roles in action:** Notice the user has both category (`homelab`) and service (`access_pangolin_ui`) roles. Pangolin evaluates all of them when making access decisions.

## 6. Granting Roles to Users

### 6.1 Internal users (within your organization)

1. Open **Project → Authorizations**.
2. Click **New**.
...
> 💡 **Combine roles strategically:** A user with `homelab` gets broad infrastructure access, but you can add `access_grafana` explicitly for clarity or if you later restrict the category rule.

### 6.2 External collaborators

If a user belongs to another organization (or should not be visible in your org's user list):

1. Go to **Project → Authorizations → External User Grant**.
...
1. Navigate to **Project → Service Users → New**.
...
> ❌ **Revoking access** is as simple as removing the authorization. Tokens already issued expire according to their lifetime; new login attempts will fail.

> 🔐 **Service user best practice:** Grant the narrowest role possible. A backup script needs `access_nextcloud` only, not `admin`.
...
2. Go to **Server Admin → Identity Providers** and open your Zitadel provider (or create it pointing to `KyleHub-Auth`).
3. In **Auth Providers → OpenID Connect**, set:
...
> ℹ️ **Community Edition 1.10.3 and earlier:** Dynamic organization policies may not be available. You can still enable auto-provisioning with a default role. Refer to the [Pangolin Auth Provider Setup](./05-pangolin-auth-provider-setup.mdx) guide for YAML-based configuration.
...
Navigate to **Server Admin → Access Control → Rules** (or open the resource's **Rules** tab).[^pangolin-rules]
...
> 💡 **Rule priority tip:** Lower numbers = higher priority. Set admin bypass to 1, service-specific to 5-9, categories to 10-99, deny-all to 100.

> ℹ️ **Community Edition UI:** The CE UI may not expose priority settings directly. Configure rules via YAML or adjust ordering in the UI list (top = first evaluated).

### 7.4 Access rule evaluation example

When a user with roles `["homelab", "access_grafana"]` visits `grafana.homelab.yourdomain.com`:

1. **Check admin bypass** (priority 1): No `admin` role → skip
2. **Check service-specific rule** (priority 5): Has `access_grafana` → **ALLOW**
3. **Check category rule** (priority 10): Has `homelab` → (not reached, already allowed)

When the same user visits `proxmox.homelab.yourdomain.com`:

1. **Check admin bypass** (priority 1): No `admin` role → skip
2. **Check service-specific rule** (priority 5): No `access_proxmox` role → skip
3. **Check category rule** (priority 10): Has `homelab` → **ALLOW** (if Proxmox didn't have a service-specific rule)
...
3. After redirect, open **Server Admin → Sessions** in Pangolin to confirm the user was auto-provisioned with the expected roles.
...
6. Remove a role in Zitadel and repeat- Pangolin should deny access on the next login.

### 7.3 Build access rules visually

1. Open your Pangolin resource and select the **Rules** tab (Community Edition users access it via **Server Admin → Access Control → Rules**).[^
pangolin-rules]
...
2. Authenticate through Zitadel.
3. After redirect, open **Server Admin → Sessions** in Pangolin to confirm the user was auto-provisioned with the expected roles.
4. Remove a role in Zitadel and repeat- Pangolin should deny access on the next login.

## 8. Managing Access Lifecycle

- **Onboarding:** create the user in Zitadel (or invite externally) ➜ issue an authorization ➜ user logs in via Pangolin ➜ Pangolin provisions the session automatically.
...
1. **Define the access pattern:**
   - Fits existing category? (e.g., new monitoring tool → `homelab` role)
   - Needs specific control? → Create `access_<servicename>` role

2. **Create service role (if needed):**
   - Navigate to **Projects → KyleHub → Roles → New**
   - Key: `access_<servicename>`, Group: `services`

3. **Update Pangolin rules:**
   - Category approach: Service matches `*.homelab.yourdomain.com` → already covered
...
5. **Optional - Direct SSO integration:**
   - Service supports OIDC/SAML? → Reuse `KyleHub-Auth` credentials
...
With this unified architecture, you maintain a single identity source, reusable permission vocabulary, and consistent enforcement across every service in your infrastructure- whether accessed through Pangolin or direct SSO integration.
